...
from axie.utils import get_nonce, load_json
    ...
    async def execute(self):
        ...
        while True:
            try:
                ...
                print(f"Waiting for transfer '{self}' to finish (Nonce:{self.nonce})...")
                # Sleep 5 seconds not to constantly send requests!
                sleep(5)
        if success:
            print(f"{self} completed! Hash: {hash} - Explorer: https://explorer.roninchain.com/tx/{str(hash)}")
